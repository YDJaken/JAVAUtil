...
import com.dy.Util.Sup.Coordinate;
...
public class Rectangle extends Config {
	...
	public String toString() {
		...
		b.append("Rectangle(").append(west).append(",").append(south).append(",").append(east).append(",").append(north)
				.append(")");
		return b.toString();
	}
	...
	boolean contains(double[] cartographic) {
		...
		if (east < west) {
			east += MathUtil.two_pi;
			if (longitude < 0.0) {
				longitude += MathUtil.two_pi;
			}
		}
		return (longitude > west || Math.abs(longitude - west) <= MathUtil.EPSILON14)
				&& (longitude < east || Math.abs(longitude - east) <= MathUtil.EPSILON14) && latitude >= this.south
				&& latitude <= this.north;
	}
}